...
RSpec.describe 'Merchant Dashboard Show Page' do
  before(:each) do
    ...
    @invoice_item_1 = InvoiceItem.create!(invoice_id: @invoice_1.id.to_s, item_id: @item_1.id.to_s, status: 1,
                                          quantity: 1, unit_price: 600, created_at: '2022-01-06 01:45:03', updated_at: '2022-01-06 01:45:03')
    @invoice_item_2 = InvoiceItem.create!(invoice_id: @invoice_2.id.to_s, item_id: @item_1.id.to_s, status: 1,
                                          quantity: 1, unit_price: 600, created_at: '2022-01-06 01:45:03', updated_at: '2022-01-06 01:45:03')
    @invoice_item_3 = InvoiceItem.create!(invoice_id: @invoice_3.id.to_s, item_id: @item_1.id.to_s, status: 2,
                                          quantity: 1, unit_price: 600)
    @invoice_item_4 = InvoiceItem.create!(invoice_id: @invoice_4.id.to_s, item_id: @item_1.id.to_s, status: 2,
                                          quantity: 1, unit_price: 600)
    @invoice_item_5 = InvoiceItem.create!(invoice_id: @invoice_5.id.to_s, item_id: @item_1.id.to_s, status: 2,
                                          quantity: 1, unit_price: 600)
  ...
  it 'has items ordered from oldest to newest' do
    ...
    expect(@invoice_1.id.to_s).to appear_before(@invoice_2.id.to_s)
  end
  ...
  it 'has a button to the merchants bulk discounts' do
    visit "/merchants/#{@merch_1.id}/dashboard"

    within '.merchant'
    click_link 'My Bulk Discounts!' do
